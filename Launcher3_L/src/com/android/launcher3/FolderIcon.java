...
public class FolderIcon extends FrameLayout implements FolderListener {
    ...
    private static final int NUM_ITEMS_IN_PREVIEW = 4;//hejianfeng modif
    ...
    static FolderIcon fromXml(int resId, Launcher launcher, ViewGroup group,
        ...
        if(folderInfo.title==null||folderInfo.title.equals("")){
        	folderInfo.title=launcher.getResources().getString(R.string.folder_hint_text);
        }
        android.util.Log.v("jeffF","hejianfeng folderInfo.title="+folderInfo.title);
        //hejianfeng add end
    ...
    private PreviewItemDrawingParams computePreviewItemDrawingParams(int index,
    	...
    	//hejianfeng add start
        int index_num = NUM_ITEMS_IN_PREVIEW - index - 1;
        float r = (index_num * 1.0f) / (NUM_ITEMS_IN_PREVIEW - 1);
        ...
        float transY;
        float transX;
        float totalScale =0.35f;
        float scaledSizemag=mIntrinsicIconSize*totalScale;
        float padding=17;
        if(index==0){
        	transY = padding;
        	transX = padding;
        	...
        	transY = padding;
        	transX = mAvailableSpaceInPreview-scaledSizemag-padding;
        }else if(index==2){
        	transY = mAvailableSpaceInPreview-scaledSizemag-padding;
        	transX = padding;
        }else if(index==3){
        	transY = mAvailableSpaceInPreview-scaledSizemag-padding;
        	transX = mAvailableSpaceInPreview-scaledSizemag-padding;
        }else{
        	 transY = mAvailableSpaceInPreview/2 *((int)(index/2))+padding;
             transX = (index%2)*mAvailableSpaceInPreview/2+padding;
        }
    ...
    private void drawPreviewItem(Canvas canvas, PreviewItemDrawingParams params) {
        canvas.save();
		android.util.Log.v("jeffF","hejianfeng getPaddingTop()="+getPaddingTop());
		android.util.Log.v("jeffF","hejianfeng mPreviewOffsetY="+mPreviewOffsetY);
        canvas.translate(params.transX+ mPreviewOffsetX, params.transY+ getPaddingTop()+mPreviewOffsetY);//hejianfeng modif
