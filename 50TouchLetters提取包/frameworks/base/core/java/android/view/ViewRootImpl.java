...
// added by bruce _hucheng for three finger screenshot begin
//import android.content.Intent;
import android.provider.Settings;
import java.lang.Math; // added by bruce for screen on gesture
// added by bruce _hucheng for three finger screenshot end
...
public final class ViewRootImpl implements ViewParent,
    ...
    // added by bruce _hucheng for screen on gesture begin
    private boolean mIsSendBroadcast = false;
    private static final int SLIDE_UP = 0;
    private static final int SLIDE_DOWN = 1;
    private static final int SLIDE_INVALID = -1;
    private int mSlideOrientation = SLIDE_INVALID;
    // added by bruce _hucheng for screen on gesture end
...
//                        mAttachInfo.mHardwareRenderer.flushLayerUpdates();
    ...
    final class ViewPostImeInputStage extends InputStage {
        ...
        private int secondDownX = 0;
        private int secondDownY = 0;
        private int thirdDownX = 0;
        private int thirdDownY = 0;
        private int forthDownX = 0;
        private int forthDownY = 0;
        // added by bruce _hucheng for screen on gesture end
        private int processPointerEvent(QueuedInputEvent q) {
            final MotionEvent event = (MotionEvent)q.mEvent;
            // added by bruce _hucheng for screen on gesture begin
            final boolean screenonEnable = Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.SCREENON_GESTURE_SETTING, 0) == 1;
            if (screenonEnable){
                final int pointCount = event.getPointerCount();
                final boolean threefingerEnable = Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.SCREENON_THREEFINGER_SETTING, 0) == 1;
                final boolean multifingerEnable = Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.SCREENON_MULTIFINGER_SETTING, 0) == 1;
                final boolean doublefingerEnable = Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.SCREENON_DOUBLEFINGER_SETTING, 0) == 1;
                
                if (multifingerEnable && (pointCount > 3)){
                    switch (event.getAction()){
                        ...
                        case MotionEvent.ACTION_POINTER_3_DOWN:
                            ...
                            firstDownX = (int)event.getX(0);
                            firstDownY = (int)event.getY(0);
                            secondDownX = (int)event.getX(1);
                            secondDownY = (int)event.getY(1);
                            thirdDownX = (int)event.getX(2);
                            thirdDownY = (int)event.getY(2);
                            forthDownX = (int)event.getX(3);
                            forthDownY = (int)event.getY(3);
                            break;
                        ...
                        case MotionEvent.ACTION_POINTER_3_UP:
                            if (mIsSendBroadcast){
                                mIsSendBroadcast = false;
                                Intent intent = new Intent().setAction("com.android.screenon.MULTI_FINGER");
                                mContext.sendBroadcast(intent);  
                            }
                            break;
                        case MotionEvent.ACTION_MOVE:
                            int firstMoveX = Math.abs((int)event.getX(0) - firstDownX);
                            int firstMoveY = Math.abs((int)event.getY(0) - firstDownY);
                            int secondMoveX = Math.abs((int)event.getX(1) - secondDownX);
                            int secondMoveY = Math.abs((int)event.getY(1) - secondDownY);
                            int thirdMoveX = Math.abs((int)event.getX(2) - thirdDownX);
                            int thirdMoveY = Math.abs((int)event.getY(2) - thirdDownY);
                            int forthMoveX = Math.abs((int)event.getX(3) - forthDownX);
                            int forthMoveY = Math.abs((int)event.getY(3) - forthDownY);
                            if (!mIsSendBroadcast 
                                && (((firstMoveX > 10) && (secondMoveX > 10) && (thirdMoveX > 10) && (forthMoveX > 10)) 
                                    || ((firstMoveY > 50) && (secondMoveY > 50) && (thirdMoveY > 50) && (forthMoveY > 50)))){
                                mIsSendBroadcast = true;
                            }
                            break;                    
                    }
                }else if (threefingerEnable && (pointCount == 3)){
                    switch (event.getAction()){
                        case MotionEvent.ACTION_DOWN:
                        case MotionEvent.ACTION_POINTER_DOWN:
                        case MotionEvent.ACTION_POINTER_2_DOWN:
                        ...
                        case MotionEvent.ACTION_POINTER_3_UP:
                            if (mIsSendBroadcast){
                                mIsSendBroadcast = false;
                                Intent intent = new Intent().setAction("com.android.screenon.THREE_FINGER");
                                mContext.sendBroadcast(intent);  
                            }
                            break;
                        case MotionEvent.ACTION_MOVE:
                            int firstMoveX = (int)event.getX(0) - firstDownX;
                            int firstMoveY = (int)event.getY(0) - firstDownY;
                            int secondMoveX = (int)event.getX(1) - secondDownX;
                            int secondMoveY = (int)event.getY(1) - secondDownY;
                            int thirdMoveX = (int)event.getX(2) - thirdDownX;
                            int thirdMoveY = (int)event.getY(2) - thirdDownY;
                            if (!mIsSendBroadcast 
                                && (((firstMoveX > 120) && (secondMoveX > 120) && (thirdMoveX > 120)
                                        && (firstMoveY < 20) && (secondMoveY < 20) && (thirdMoveY < 20)) 
                                    || ((firstMoveY > 120) && (secondMoveY > 120) && (thirdMoveY > 120)
                                        && (firstMoveX < 80) && (secondMoveX < 80) && (thirdMoveX < 80)))
                                ){
                                mIsSendBroadcast = true;
                            }
                            break;                    
                    }
                }else if (doublefingerEnable && (pointCount == 2)){
                    switch (event.getAction()){
                        case MotionEvent.ACTION_DOWN:
                        case MotionEvent.ACTION_POINTER_DOWN:
                        case MotionEvent.ACTION_POINTER_2_DOWN:
                        ...
                        case MotionEvent.ACTION_POINTER_2_UP:
                        case MotionEvent.ACTION_POINTER_3_UP:
                            mSlideOrientation = SLIDE_INVALID;
                            break;
                        case MotionEvent.ACTION_MOVE:
                            int firstMoveX = Math.abs((int)event.getX(0) - firstDownX);
                            int secondMoveX = Math.abs((int)event.getX(1) - secondDownX);
                            int tempFirstDownY = (int)event.getY(0);
                            int tempSecondDownY = (int)event.getY(1);
                            if ((tempFirstDownY < firstDownY - 20) && (tempSecondDownY < secondDownY - 20)){
                                mSlideOrientation = SLIDE_UP;
                                firstDownY = tempFirstDownY;
                                secondDownY = tempSecondDownY;
                            }else if ((tempFirstDownY > firstDownY + 20) && (tempSecondDownY > secondDownY + 20)){
                                mSlideOrientation = SLIDE_DOWN;
                                firstDownY = tempFirstDownY;
                                secondDownY = tempSecondDownY;
                            }else {
                                mSlideOrientation = SLIDE_INVALID;
                            ...
                            if (mSlideOrientation != SLIDE_INVALID){
                                handleScreenOnVolume(mSlideOrientation);
                            }
                            break;                    
                    }
                }
        ...
        // added by bruce _hucheng for screen on gesture begin
        private void handleScreenOnVolume(int orientation){
            final AudioManager manager = getAudioManager();
            Log.i("bruce_nan", "handleScreenOnVolume_nfl: orientation = " + orientation);
            if (manager != null){
                if (orientation == SLIDE_DOWN){
                    manager.adjustStreamVolume(AudioManager.STREAM_MUSIC,AudioManager.ADJUST_LOWER,    
                        AudioManager.FX_FOCUS_NAVIGATION_UP);
                }else if (orientation == SLIDE_UP){
