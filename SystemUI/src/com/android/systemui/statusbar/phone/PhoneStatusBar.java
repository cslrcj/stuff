...
import android.content.pm.ApplicationInfo;//hejianfeng add 
//hejianfeng add start for lockwallpaper
import java.io.File;
...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    //begin 添加下面一行hejianfeng
    PanelHolder holder;//成员变量
    //end
    // ================================================================================
...
//        PanelHolder holder = (PanelHolder) mStatusBarWindow.findViewById(R.id.panel_holder);
        ...
        wallpapers=mContext.getResources().getStringArray(R.array.wallpapers);
      //hejianfeng add end 
		...
		filter.addAction("com.magcomm.lockwallpapers");//hejianfeng add 
    ...
    private BroadcastReceiver mDemoReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
			...
			//hejianfeng add start
            else if("com.magcomm.lockwallpapers".equals(action)){
            	 Log.v("jeff","hejianfeng com.magcomm.lockwallpapers");
            	 Bundle bundle = intent.getExtras();
            	 position=bundle.getInt("item");
            }
            //hejianfeng add end 
        }
    };
...
//hejianfeng add start 
    ...
    private void updateKeyguardState(boolean goingToFullShade, boolean fromShadeLocked) {
        ...
        position = Settings.System.getInt(mContext.getContentResolver(), Settings.System.MAGCOMM_LOCK_WALLPAPER_ID, 1);
        Log.i("bruce_nan", "PhoneStatusBar_nfl_02: position = " + position);
        if ((mState == StatusBarState.KEYGUARD || mState == StatusBarState.SHADE_LOCKED)&& android.os.SystemProperties
            .get("ro.launcher_lock_wallpapers").equals("1")) {
        	//begin 添加下面一行hejianfeng 
        	Log.v("jeff","hejianfeng setBackgroundResource");
            Log.i("bruce_nan", "PhoneStatusBar_nfl_03");
            Drawable mDrawable=null;
            if(position==0){
            	mDrawable=getLockWallpaper();
            }
            if(position==0){
            	holder.setBackground(mDrawable);
            }else{
            	holder.setBackgroundResource(getResID(wallpapers[position]));
            }
            //holder.setBackgroundResource(getResID(wallpapers[wallpaper_id]));
            //end
            mScrimController.setKeyguardShowing(true);
            mIconPolicy.setKeyguardShowing(true);
        } else {
            Log.i("bruce_nan", "PhoneStatusBar_nfl_04");
        	//begin 添加下面一行hejianfeng
