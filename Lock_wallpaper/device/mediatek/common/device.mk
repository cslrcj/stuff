...
#add by y.haiyang for magcomm-framework
PRODUCT_PACKAGES += \
    magcomm-framework
#add by y.haiyang for magcomm-framework
PRODUCT_BOOT_JARS += \
    magcomm-framework
#add by y.haiyang for magcomm res
PRODUCT_PACKAGES += magcomm-res
...
#hejianfeng add for tuopan start 
ifeq ($(strip $(MAGCOMM_LAUNCHER3)), yes)
	PRODUCT_PACKAGES += Launcher3
	PRODUCT_PACKAGES += IphoneTheme
else
	PRODUCT_PACKAGES += Launcher3
endif
#hejianfeng add for tuopan end 
#hejianfeng add for lock wallpapers
ifeq ($(strip $(MAGCOMM_LOCK_WALLPAPERS)), yes)
	PRODUCT_PROPERTY_OVERRIDES += ro.launcher_lock_wallpapers=1
    PRODUCT_PACKAGES += LockWallpapers
endif
#Add for CCCI Lib
...
$(warning copy COMBO ANT RAM code)
...
$(warning copy  CONN ANT RAM CODE)
...
# added by bruce for CIT begin
PRODUCT_PACKAGES += CIT \
                    libcitjni

ifeq ($(strip $(MTK_2SDCARD_SWAP)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.mount.swap=1
else
  PRODUCT_PROPERTY_OVERRIDES += ro.mount.swap=0
endif
# added by bruce for CIT end

# added by bruce for touchletters
ifeq ($(strip $(MAGCOMM_TOUCHLETTER_SUPPORT)),yes)
  PRODUCT_PACKAGES += TouchLetters
endif
#dengjianzhang@20150920 start
ifeq ($(strip $(MAGCOMM_SHUTTER_SOUND_SUPPORT)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.shutter_sound_support=1
endif
ifeq ($(strip $(MAGCOMM_CAMERA_TIME_STAMP_SUPPORT)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.time_stamp_support=1
endif
ifeq ($(strip $(MAGCOMM_CAMERA_TOUCH_PHOTOGRAPH_SUPPORT)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.touch_photograph_support=1
endif
