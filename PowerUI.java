...
public class PowerUI extends SystemUI {
    ...
    private final class Receiver extends BroadcastReceiver {
		...
		// added by bruce for usb ring begin
                Slog.d("bruce_nan", "onReceive_nfl_01: oldPlugged = " + oldPlugged + "; plugged = " + plugged);
               	if (!oldPlugged && plugged){
                    playUsbRingSound();
               	} 
                // added by bruce for usb ring end
    ...
    // added by bruce for usb ring begin
    void playUsbRingSound() {
        final ContentResolver cr = mContext.getContentResolver();
        final String soundPath = "/system/media/audio/ui/UsbConnected.ogg";
                
        Slog.d("bruce_nan", "playUsbRingSound_nfl: soundPath = " + soundPath);
        if (soundPath != null) {
            final Uri soundUri = Uri.parse("file://" + soundPath);
            Slog.d("bruce_nan", "playUsbRingSound_nfl_01: soundUri = " + soundUri);
            if (soundUri != null) {
                final Ringtone sfx = RingtoneManager.getRingtone(mContext, soundUri);
                /// M: If no NotificationPlayer,Use Ringtone.
                Slog.d("bruce_nan", "playUsbRingSound_nfl_02");
                //if (sfx != null && mNP == null) {
                if (sfx != null) {
                    Slog.d("bruce_nan", "playUsbRingSound_nfl_03");
    ...
    // added by bruce for usb riing end
}
