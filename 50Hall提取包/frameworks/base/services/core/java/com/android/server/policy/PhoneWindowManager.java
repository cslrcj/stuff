...
// added by bruce _hucheng for take picture begin
import android.hardware.Camera;
import android.hardware.Camera.Parameters;
import android.app.Instrumentation;
// added by bruce _hucheng for take picture end
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void init(Context context, IWindowManager windowManager,
		...
		// added by bruce _hucheng for screenon gesture begin
        IntentFilter threeFingerEventFilter = new IntentFilter();
        threeFingerEventFilter.addAction("com.android.screenon.THREE_FINGER");
        context.registerReceiver(mScreenOnReceiver, threeFingerEventFilter);
        
        IntentFilter multiFingerEventFilter = new IntentFilter();
        multiFingerEventFilter.addAction("com.android.screenon.MULTI_FINGER");
        context.registerReceiver(mScreenOnReceiver, multiFingerEventFilter);
		// del by bruce _bruce for double finger begin
		/*
        IntentFilter doubleFingerEventFilter = new IntentFilter();
        doubleFingerEventFilter.addAction("com.android.screenon.DOUBLE_FINGER");
        context.registerReceiver(mScreenOnReceiver, doubleFingerEventFilter);
		*/
		// del by bruce _bruce for double finger end
        // added by bruce _hucheng for screenon gesture end
    ...
    /** {@inheritDoc} */
    // added by hucheng for bruce _screenon gesture begin
    private static long startHomeKeyTime = 0;
    private static long internalHomeKeyTime = 0;
    // added by hucheng for bruce _screenon gesture end
    @Override
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            // added by bruce _hucheng for screenon gesture begin
		    boolean screenOnEnable = Settings.System.getInt(mContext.getContentResolver(),
                            Settings.System.SCREENON_GESTURE_SETTING,0) == 1;
            if (!down && screenOnEnable){
                if (startHomeKeyTime == 0){
                    startHomeKeyTime = System.currentTimeMillis();
                }else {
                    internalHomeKeyTime = System.currentTimeMillis() - startHomeKeyTime;
                    Log.i("bruce_nan", "PhoneWindowManager_interceptKeyBeforeDispatching: startHomeKeyTime = " + startHomeKeyTime
                        + "; internalHomeKeyTime = " + internalHomeKeyTime);
                    if ((internalHomeKeyTime > 100) && (internalHomeKeyTime <= 500)){
                        boolean doubleClickEnable = Settings.System.getInt(mContext.getContentResolver(),
                            Settings.System.SCREENON_DOUBLECLICK_SETTING,0) == 1;
                        if (doubleClickEnable){
                            Log.i("bruce_nan", "PhoneWindowManager_interceptKeyBeforeDispatching_nfl: start lock screen");
                            //Intent intent = new Intent().setAction("com.android.screenon.DOUBLE_CLICK");
                            //mContext.sendBroadcast(intent); 
                            //lockNow(null);
                            sendKeyCode(KeyEvent.KEYCODE_POWER);
                        }
                        startHomeKeyTime = 0;
                        internalHomeKeyTime = 0;
            ...
            //add by lixiang 20160824
            boolean fsc = Settings.System.getInt(mContext.getContentResolver(), Settings.System.FREE_SCREENSHOTS_SETTING, 0) == 1;
            Log.i("lixiang", "fsc = " + fsc);
            Log.v("jeff","KeyEvent.KEYCODE_MENU ="+repeatCount);
            //hejianfeng add start hucheng free screenshot
            if(down && repeatCount>0 && SystemProperties.get("ro.magcomm_screen_shot_support").equals("1") && fsc){
            	  ComponentName cn = new ComponentName("com.android.systemui",
                          "com.android.systemui.screenshot.magcomm.ScreenShotActivity");
            ...
            //hejianfeng add end hucheng free screenshot
	...
	private boolean isCameraRunning() {
		...
		if (!currentPackageName.equals("") && !currentClassName.equals("") && currentPackageName.equals("com.android.gallery3d") && (currentClassName.equals("com.android.camera.CameraLauncher") || currentClassName.equals("com.android.camera.Camera") ||currentClassName.equals("com.android.camera.SecureCameraActivity")||currentClassName.equals("com.android.camera.CaptureActivity"))) {
			return true;
		}
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        final boolean canceled = event.isCanceled();
        final int keyCode = event.getKeyCode();
        Log.i("lixiang", "5093-keyCode = " + keyCode);
        ...
        // added by bruce _hucheng for double tp wake up screen begin
	    if (keyCode == KeyEvent.KEYCODE_TP_DBL) {
	        boolean isOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHLETTER_ONOFF, 0) == 0 ? false: true;
	        boolean isOpen1 = Settings.System.getInt(mContext.getContentResolver(), Settings.System.DOUBLE_SCREEN_ONOFF, 0) == 0 ? false: true;
            Log.i("bruce_nan", "interceptKeyBeforeQueueing_nfl_01: isOpen = " + isOpen + "; isOpen1 = " + isOpen1);
	        if (isOpen && isOpen1)
                policyFlags |= WindowManagerPolicy.FLAG_WAKE;
        }
	    // added by bruce _hucheng for double tp wake up screen end
        /// M: Remove this log.
	    ...
	    case KeyEvent.KEYCODE_HALL:		    
		    ...
		    isHallMenuChecked = Settings.System.getInt(mContext.getContentResolver(), Settings.System.HALL_SETTING, 0);
		    ...
		    if (down && repeatCount == 0) {
				...
				mHall_7799 = true;
				Settings.System.putInt(mContext.getContentResolver(), Settings.System.HALL_CLOSE, 1);
	     ...
	     case KeyEvent.KEYCODE_HALL_RELEASE:
		    ...
		    if(down && repeatCount == 0){
				...
				mHall_7799 = false;
				Settings.System.putInt(mContext.getContentResolver(), Settings.System.HALL_CLOSE, 0);
				/** hejianfeng delete 
				boolean incomingRinging = false;
				boolean isAnswerRingCall = false;
				boolean isRinging = false;
		...
		break;
	    // hejianfeng@20150601 end
        //bruce _hucheng add touch letter
	    case KeyEvent.KEYCODE_TP_C:
        Log.i("bruce_nan", "interceptKeyBeforeQueueing_nfl_C: down = " + down);
        if (!down)
        {
            Log.d("bruce_nan","***************KEYCODE_KEY_TP_LETTER_C********************");
            Boolean isOpen = false;
            Log.d("bruce_nan","11111isOpen = "+isOpen);
            isOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHLETTER_ONOFF, 0) 
                       == 0 ? false: true;
            Log.d("bruce_nan","2222isOpen = "+isOpen);
            if (isOpen){
                try{
                    Intent intent = new Intent();
                    intent.setClassName("com.magcomm.touch", "com.magcomm.touch.TouchLetterActivity");
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|
				    Intent.FLAG_ACTIVITY_NO_HISTORY |
				    Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT);
                    intent.putExtra("letter", "c");
                    ComponentName comp = new ComponentName("com.magcomm.touch", "com.magcomm.touch.TouchLetterActivity"); 
                    intent.setComponent(comp); 
                    mContext.startActivity(intent);
                }catch (Exception ex){
                    Intent mIntent = new Intent("magcomm.action.TOUCH_LETTER");
                    mIntent.putExtra("letter", "c");
		            mContext.sendBroadcast(mIntent);
                }
        ...
        break;
	    case KeyEvent.KEYCODE_TP_E:
        Log.i("bruce_nan", "interceptKeyBeforeQueueing_nfl_E: down = " + down);
        if (!down)
        {
		    Log.d("bruce_nan","***************KEYCODE_KEY_TP_LETTER_E********************");
            Boolean isOpen = false;
            Log.d("bruce_nan","11111isOpen = "+isOpen);
            isOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHLETTER_ONOFF, 0)
                       == 0 ? false: true;
            ...
            if (isOpen){
                try{
                    ...
                    Intent mIntent = new Intent("magcomm.action.TOUCH_LETTER");
                    mIntent.putExtra("letter", "e");
                    mContext.sendBroadcast(mIntent);
                }
            }
        }
        break;
        case KeyEvent.KEYCODE_TP_M:
        Log.i("bruce_nan", "interceptKeyBeforeQueueing_nfl_M: down = " + down);
        if (!down)
        {
            Log.d("bruce_nan","***************KEYCODE_KEY_TP_LETTER_M********************");
            Boolean isOpen = false;
            Log.d("bruce_nan","11111isOpen = "+isOpen);
            isOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHLETTER_ONOFF, 0)
                       == 0 ? false: true;
        ...
        break;
        case KeyEvent.KEYCODE_TP_O:
        Log.i("bruce_nan", "interceptKeyBeforeQueueing_nfl_O: down = " + down);
        if (!down)
        {
            Log.d("bruce_nan","***************KEYCODE_KEY_TP_LETTER_O********************");
            Boolean isOpen = false;
            Log.d("bruce_nan","11111isOpen = "+isOpen);
            isOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHLETTER_ONOFF, 0)
                       == 0 ? false: true;
            ...
            if (isOpen){
                try{
                    ...
                    Intent mIntent = new Intent("magcomm.action.TOUCH_LETTER");
                    mIntent.putExtra("letter", "o");
                    mContext.sendBroadcast(mIntent);
                }
            }
        }
        break;
        case KeyEvent.KEYCODE_TP_W:
        case KeyEvent.KEYCODE_TP_UP:
        case KeyEvent.KEYCODE_TP_DOWN:
        Log.i("bruce_nan", "interceptKeyBeforeQueueing_nfl_W: down = " + down);
        if (!down)
        {
            Log.d("bruce_nan","***************KEYCODE_KEY_TP_LETTER_w********************");
            Boolean isOpen = false;
            Boolean isSwitchMusicOpen = false;
            Log.d("bruce_nan","11111isOpen = "+isOpen);
            isOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHLETTER_ONOFF, 0)
                       == 0 ? false: true;
            isSwitchMusicOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHSWITCH_MUSIC_ONOFF, 0)
                       == 0 ? false: true;
            Log.d("bruce_nan","2222isOpen = "+isOpen);
            if (isOpen){
                try{
                   ...
                   intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|
                             Intent.FLAG_ACTIVITY_NO_HISTORY |
                             Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT);
                   if (keyCode == KeyEvent.KEYCODE_TP_W){
                       intent.putExtra("letter", "w");
                   }else if (keyCode == KeyEvent.KEYCODE_TP_UP){
                       intent.putExtra("letter", "up");
                   }else if (keyCode == KeyEvent.KEYCODE_TP_DOWN){
                       intent.putExtra("letter", "down");
                   }
                   ComponentName comp = new ComponentName("com.magcomm.touch", "com.magcomm.touch.TouchLetterActivity"); 
                   intent.setComponent(comp); 
                   mContext.startActivity(intent);
       ...
       // added by bruce _hucheng for touchletter begin
       case KeyEvent.KEYCODE_TP_LEFT:
       {
           boolean isOpen = false;
           boolean isSwitchMusicOpen = false;
           isOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHLETTER_ONOFF, 0)
           ...
           isSwitchMusicOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHSWITCH_MUSIC_ONOFF, 0)
                      == 0 ? false: true;
           Log.i("bruce_nan", "interceptKeyBeforeQueueing_nfl_01: down = " + down + "; isOpen = " + isOpen + "; isSwitchMusicOpen = " + isSwitchMusicOpen);
		   if(!down && isOpen &&  isSwitchMusicOpen){
               Log.d("bruce_nan",TAG + " intercept before Queue:TP_LEFT");
               Intent intentLeft1 = new Intent();
               intentLeft1.setClassName("com.android.music", "com.android.music.MediaPlaybackService");                
               mContext.startService(intentLeft1);
               ...
               Intent intentLeft2 = new Intent();
               intentLeft2.setAction("com.android.music.musicservicecommand");
               intentLeft2.putExtra("command", "previous");
               mContext.sendBroadcast(intentLeft2);
		       result = ACTION_PASS_TO_USER;	
		   }
        }
		break;
	        case KeyEvent.KEYCODE_TP_RIGHT:
	        {
                boolean isOpen = false;
                boolean isSwitchMusicOpen = false;
                isOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHLETTER_ONOFF, 0)
                ...
                isSwitchMusicOpen = Settings.System.getInt(mContext.getContentResolver(), Settings.System.TOUCHSWITCH_MUSIC_ONOFF, 0)
                       == 0 ? false: true;
                Log.i("bruce_nan", "interceptKeyBeforeQueueing_nfl_02: down = " + down + "; isOpen = " + isOpen + "; isSwitchMusicOpen = " + isSwitchMusicOpen);
		        if(!down && isOpen &&  isSwitchMusicOpen){
	                Log.d("bruce_nan",TAG + " intercept before Queue:TP_RIGHT");
                    Intent intentRight1 = new Intent();
                    intentRight1.setClassName("com.android.music", "com.android.music.MediaPlaybackService");
                    mContext.startService(intentRight1);
                    ...
                    Intent intentRight2 = new Intent();
                    intentRight2.setAction("com.android.music.musicservicecommand");
                    intentRight2.putExtra("command", "next");
                    mContext.sendBroadcast(intentRight2);
		            result = ACTION_PASS_TO_USER;
		        }
		    }
    		    break;
            // added by hucheng for touchletter end
    		case KeyEvent.KEYCODE_F3:
                //case KeyEvent.KEYCODE_F4:
                // modified by bruce for touch photo begin
                boolean touchPhotoEnabled = Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.MAGCOMM_TOUCH_PHOTO, 0) == 1;
                Log.i("bruce_nan", "PhoneWindowManager_KEYCODE_F3_nfl: touchPhotoEnabled = " + touchPhotoEnabled);
                if (touchPhotoEnabled){
                    if (isCameraRunning() && !down){
                        sendKeyCode(KeyEvent.KEYCODE_CAMERA);
                        result &= ~ACTION_PASS_TO_USER;
                    }
                }
               // modified by bruce _hucheng for touch photo begin
            	break;  
    ...
    BroadcastReceiver mScreenOnReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.i("bruce_nan", "mScreenOnReceiver_nfl: action = " + intent.getAction());
            if ("com.android.screenon.THREE_FINGER".equals(intent.getAction())){
                takeScreenshot();
            }else if ("com.android.screenon.MULTI_FINGER".equals(intent.getAction())){
				if (!isCameraRunning()){
                Intent screenonIntent = new Intent();
                screenonIntent.setClassName("com.android.gallery3d","com.android.camera.CameraLauncher");
                screenonIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                ComponentName comp = new ComponentName("com.android.gallery3d","com.android.camera.CameraLauncher"); 
                screenonIntent.setComponent(comp); 
                mContext.startActivity(screenonIntent);
				}
            } /*else if ("com.android.screenon.DOUBLE_FINGER".equals(intent.getAction())){
                Bundle bundle = intent.getExtras();
                int slide_orientation = -1;
                slide_orientation = bundle.getInt("slide_orientation");
                Log.i("bruce_nan", "mScreenOnReceiver_nfl_01: slide_orientation = " + slide_orientation);
                if (slide_orientation == 0){
                    sendKeyCode(KeyEvent.KEYCODE_VOLUME_UP);
                }else if (slide_orientation == 1){
                    sendKeyCode(KeyEvent.KEYCODE_VOLUME_DOWN);
                }
            } */ // del by bruce for double finger
        }
    };
    // added by bruce _hucheng for screenon gesture end
